...
def nameColorByNetwork(x, palette=None, namecol="name", other_color=None):
    """
    ...
    if not other_color: other_color = palette[2]
    
    assert namecol in x.columns, 'namecol should be a valid column'
    
    # mask = x['network'] == 'mainnet'
    style = pd.DataFrame('', index=x.index, columns=x.columns)
    style[namecol] = x['network'].apply(lambda c: 'color:' + color_dict.get(c, other_color)) # np.where(mask, 'color:'+palette[0], 'color:'+palette[1])

    return style

def custom_pct_format(x, prec=2):
    if x == 1:
        return '(100%)'
    else:
        return f'({x*100:.2g}%)'
    
def add_pct_col(df: pd.DataFrame, col, col_total, col_pct = None, main_format=lambda x: f'{x:.0f}', nan_replace='–'):
    col_pct = col_pct or f'{col} (pct)'
    div = df[col]/df[col_total]
    df[col_pct] = df[col].apply(main_format) + ' ' + div.apply(custom_pct_format)
